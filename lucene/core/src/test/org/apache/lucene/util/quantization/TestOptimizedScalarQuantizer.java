...
public class TestOptimizedScalarQuantizer extends LuceneTestCase {
  ...
  /**
   * Quantizing one vector at two bit widths in a single call must produce exactly what two
   * independent single-width calls produce. The merge path relies on this: it replaces a {@code
   * scalarQuantize} in the writer and another one in the reader with a single {@code
   * multiScalarQuantize}, and the records that reach the index have to be the same bytes.
   */
